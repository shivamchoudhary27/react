...
function QuestionsPageNav(prop) {
  ...
  function generateButtons(totalPages, currentPage, changePage) {
    ...
    for (let i = 1; i <= totalPages; i += 1) {
      ...
      if (i === currentPage + 1) {
        ...
        buttons.push(<button type="button" key={i} value={i} className="quiz-navigation-btn" disabled>{i}</button>);
      }
    }
    else{
      if (i === currentPage + 1) {
        ...
        buttons.push(<button type="button" key={i} value={i} className="quiz-navigation-btn" onClick={changePage}>{i}</button>);
      }
    }
    }
    return buttons;
  ...
  return (
    <div className="quiz-navigation-content">
      ...
      <button type="button" className="quiz-preview-btn" onClick={props.finisAttempt}>Finish attempt..</button>
    </div>
  );
}
