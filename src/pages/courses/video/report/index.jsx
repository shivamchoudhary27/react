                    ...
                    {show === true ? <Errordiv cstate={show} msg="Something went wrong" /> :
                        <div className="pt-5">
                            <select onChange={handleChange} id="selct_item">
                                ...
                                {course != undefined && course.map((item) => (
                                    <option value={item.id} key={item.id}>{item.fullname}
                                    </option>
                                ))
                                }
                            </select>
                            <div>
                                ...
                                <table className="table">
                                    ...
                                    <tbody>
                                        {videos.length > 0 && videos.map((modname) => (
                                            <tr key={modname.id} >
                                                {videoStatusColumns(modname.id, modname.contentfiles[0].fileurl, modname.name, modname.course)}
                                            </tr>
                                        ))
                                        }
                                    </tbody>
